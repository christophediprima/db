...
/// compiles under `--all-features` and fails the default build with E0433 — which
/// is exactly how it reached CI. `fluree-db-api` depends on the indexer
/// unconditionally, and the server already sources its other clap defaults from
/// this module, so routing it here keeps one definition and one import path.
