...
pub struct AppState {
    ...
    /// after this `AppState` is behind an `Arc` — unlike `materialize_worker`
    /// above, which is spawned during construction.
    ///
    /// Under Raft the worker starts and stops with leadership, so this is a
    /// slot that gets cleared and refilled, not a `OnceLock`. Separately
    /// `Arc`ed so the Raft leader-watcher closure can fill it: that closure is
    /// built before the state is wrapped, and capturing the state would be a
    /// cycle.
    ///
