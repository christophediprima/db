...
`fluree server` keeps BM25 indexes fresh automatically — see [Background
Maintenance Worker](#background-maintenance-worker). Embedded and CLI users
sync explicitly.

### Syncing

An index is only as current as its watermark. To sync one yourself:
...
`fluree server` spawns a `Bm25MaintenanceWorker` on every write node (not on
peers, which forward writes and so see no commits). It:

- adopts every persisted, non-retracted, **tracked** BM25 record on start-up,
  and syncs the ones already behind their source ledger — so indexes created out
  of process (`fluree bm25 create`) are picked up, and nothing has to be
  re-registered after a restart;
- re-syncs an index when any ledger it depends on commits, debounced (100ms by
  default) so a burst of commits collapses into one sync;
- auto-registers newly created BM25 indexes and drops retracted ones, from the
  same event stream;
- bounds concurrent syncs (4 by default).

**The event bus is in-process.** Only the process that *writes* a commit
publishes the event, so the worker has to live in the writer — the server. A
one-shot `fluree bm25 sync`, or a daemon pointed at the same storage
directory, is deaf to those events; that's what the `--stale` polling loop
under [CLI Management](#cli-management) is for.

Registration and status are exposed over HTTP as
`POST /v1/fluree/bm25/track`, `POST /v1/fluree/bm25/untrack`, and
`GET /v1/fluree/bm25/tracking` — see
[API Endpoints](../api/endpoints.md#post-api_base_urlbm25track). `track` is
worth calling right after an out-of-process `fluree bm25 create`: it adopts the
new index immediately instead of waiting for the next server restart.

#### Opting an index out

Maintenance is **per index and opt-out**, controlled by a `tracked` flag stored
on the index record — so it survives restarts and any process reading the
nameservice can see it:

```bash
# Built, but nobody auto-syncs it: a point-in-time snapshot, or one whose
# resync is too expensive to run on every commit.
fluree bm25 create --name archive-search --ledger docs:main -f q.json --no-track

fluree bm25 list                # TRACKED column: yes / NO
fluree bm25 list --untracked    # aliases a `sync` cron still has to cover
fluree bm25 list --stale --untracked | while read -r i; do
  fluree bm25 sync --index "$i"
done
```

```rust
let config = Bm25CreateConfig::new("archive-search", "docs:main", query).with_tracked(false);

// Or flip it later (this is what the HTTP routes do):
fluree.set_bm25_tracked("archive-search:main", true).await?;
```

The default is tracked — an index nobody keeps fresh silently returns stale
results, so opting out should be a decision. Indexes created before this flag
existed read as tracked.

Embedding the worker directly:

```rust
use std::sync::Arc;
use fluree_db_api::Bm25MaintenanceWorker;

let fluree = Arc::new(FlureeBuilder::file("/path/to/data").build_client().await?);

let worker = Bm25MaintenanceWorker::new(Arc::clone(&fluree));
let handle = worker.handle();
let task = tokio::spawn(worker.run());

// Optional — start-up enumeration already adopted persisted indexes.
handle.register_graph_source("article-search:main").await?;
...
| Command | Description |
|---------|-------------|
| `fluree bm25 create --name <n> --ledger <l> -f <index-query.json>` | Build a BM25 index over a source ledger from an indexing query. Add `--no-track` to opt out of server-side auto-maintenance |
| `fluree bm25 list [--stale] [--untracked]` | List indexes with their source ledger, `INDEX_T`/`LEDGER_T`, staleness and `TRACKED`. Either flag switches to script mode: `name:branch` per line, filtered |
...
A maintenance loop syncs every stale index a server isn't already maintaining:
...
`TRACKED` is the persisted *intent*. Whether a worker is actually running — and
in which process — is per-server runtime state this local CLI cannot see; ask
the server with `GET /v1/fluree/bm25/tracking`, which reports its pid and its
live registrations.
