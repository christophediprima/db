...
`--bm25-auto-sync` is deployment-wide, but individual indexes can opt out of it — see [Opting an index out](#opting-an-index-out).
...
Unless the server runs with `--bm25-auto-sync` (see [Background Maintenance Worker](#background-maintenance-worker)), a BM25 index is not updated when its source ledger changes, and must be synced explicitly:
...
The worker also adopts the indexes that already exist when it starts, rather than only the ones created while it is running, and syncs any that its source ledger has already moved past. Without that catch-up an index that fell behind while the server was down would wait for the next commit to its source — on a ledger that has gone quiet, indefinitely.

#### Opting an index out

The choice above is not all-or-nothing. Each index carries a `tracked` flag, so one expensive index can be exempted from `--bm25-auto-sync` while the rest of the deployment keeps syncing on commit:

```bash
# Create an index the worker will leave alone.
fluree bm25 create --name archive-search --ledger archive:main -f query.json --no-track

# Or change it later.
fluree bm25 untrack --index archive-search:main
fluree bm25 track   --index archive-search:main
```

Over HTTP that is `POST /v1/fluree/bm25/track` and `POST /v1/fluree/bm25/untrack`; from Rust, `Bm25CreateConfig::with_tracked(false)` and `Fluree::set_bm25_tracked`.

The flag lives on the graph-source record rather than in server config, so it survives a restart and any process reading the nameservice can see it — `fluree bm25 list` shows it as a `TRACKED` column, and `fluree bm25 list --stale --untracked` prints exactly the indexes nothing is keeping fresh, which is what a scheduled sync job should walk. An index created before the flag existed reads as tracked, so upgrading does not silently stop maintaining anything.

An untracked index is otherwise unchanged: it still answers queries, and `fluree bm25 sync` still advances it.

To see whether a given server is actually running a worker, and which indexes it has adopted, `GET /v1/fluree/bm25/tracking`. A durable `tracked: true` still syncs nothing if no node runs a worker.
